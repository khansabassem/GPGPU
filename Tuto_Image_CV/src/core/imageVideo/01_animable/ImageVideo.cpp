...
ImageVideo::ImageVideo(uint w , uint h , string nameVideo) :
	Animable_I<uchar4>(w, h, "ImageVideo_OMP_rgba_uchar4"), //
	nameVideo(nameVideo), //
	capture(nameVideo), //
	matRGBA(h, w, CV_8UC1)
    {
 ...
 */
void ImageVideo::processForAutoOMP(uchar4* tabPixels , uint w , uint h , const DomaineMath& domaineMath)
    {
    memcpy(tabPixels, this->ptrTabPixelVideo, sizeImage);
...
#pragma omp parallel for
	for (int i = 0; i < h; i++)
		...
		math.colorIJ(&tabPixels[s], i, j, t);
		}
	    }
	}
 ...
 * Image non zoomable : domaineMath pas use ici
 */
void ImageVideo::processEntrelacementOMP(uchar4* tabPixels , uint w , uint h , const DomaineMath& domaineMath)
    {
    memcpy(tabPixels, this->ptrTabPixelVideo, sizeImage);
...
#pragma omp parallel
		...
		math.colorIJ(&tabPixels[s], i, j, t); // ici pas besoin de i, j (et t) optimisation possible
