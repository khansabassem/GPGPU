 ...
 *
 */
class Newton3D_RGBA: public gpu::SurfaceStrip_RGBA_uchar4
    {
	/*--------------------------------------*\
	|*		Constructor		*|
	 \*-------------------------------------*/
...
//	Newton3D_RGBA(const Grid& grid , int w , int h , const gpu::DomainMath3D& domaineMath , float dt , int n);
	Newton3D_RGBA(const Grid& grid , uint w , uint h , uint nMin, uint nMax, const gpu::DomainMath3D& domaineMath);

	virtual ~Newton3D_RGBA(void);
...
//	gpu::Variateur<int> variateurT;
//	Variateur<int> variateurN;

	gpu::Variateur<int> variateurN;
